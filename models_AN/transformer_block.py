...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., depth=0, compression_factor=2, block_depth=3):
        super().__init__()
        self.depth = depth
    ...
    def forward(self, x):
        ...
        if self.depth >= self.block_depth and self.depth <= 14-self.block_depth:
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., depth=0, compression_factor=2, block_depth=3):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, depth=0, compression_factor=2, block_depth=3):
        super().__init__()
        self.depth = depth
        self.compression_factor = compression_factor
        self.block_depth = block_depth
        #print(compression_factor, depth)  
        #self.norm1 = norm_layer(dim)
        self.norm1_large = norm_layer(dim)
        self.norm1_small = norm_layer(dim)

        self.attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, depth=depth, compression_factor=compression_factor, block_depth=block_depth)
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, depth=depth, compression_factor=compression_factor, block_depth=block_depth)
