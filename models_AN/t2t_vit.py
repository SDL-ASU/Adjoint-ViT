...
class T2T_ViT(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, token_dim=64, compression_factor=2, block_depth=3):
        super().__init__()
        print("T2T_ViT")
        ...
        dan_architecure = [1,1,1,1,6,3,2,2,6,6,1,1,1,1]
        dan_architecure = [1,1,1,1,3,2,3,1,3,1,1,1,1,1]
        #dan_architecure = [1,1,2,1,6,3,2,2,6,6,1,3,1,1]

        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, depth=i, compression_factor=dan_architecure[i], block_depth=block_depth)
    ...
    def forward_features(self, x):
        ...
        x_small[:,:,self.embed_dim//self.compression_factor:] = 0
