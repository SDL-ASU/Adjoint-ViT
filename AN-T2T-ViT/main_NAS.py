...
import models_NAS
from models_NAS.t2t_vit import AdjoinedLossNAS
...
from pytorch_image_models.data import ImageDataset, create_loader, resolve_data_config, Mixup, FastCollateMixup, AugMixDataset
from pytorch_image_models.models import load_checkpoint, create_model, resume_checkpoint, convert_splitbn_model
from pytorch_image_models.utils import *
from pytorch_image_models.loss import LabelSmoothingCrossEntropy, SoftTargetCrossEntropy, JsdCrossEntropy
from pytorch_image_models.optim import create_optimizer, create_optimizer_search
from pytorch_image_models.scheduler import create_scheduler
from pytorch_image_models.utils import ApexScaler, NativeScaler
...
# Architecture Search
parser.add_argument('--compression_factor', type=int, default=[1,2,3], help='')
parser.add_argument('--block-depth', type=int, default=4, help='')
parser.add_argument('--get_architecture', action='store_true', default=False)
parser.add_argument('--is_NAS_training', type=str, default='True')
parser.add_argument('--gumbel-weight-decay', type=float, default=0.05,
					help='weight decay (default: 0.005 for adamw)')
parser.add_argument('--search_data_percent', type=float, default=0.2, help='')
parser.add_argument('--latency_coefficient', type=float, default=1e-12, help='')
...
def main():
	...
	if args.get_architecture:
		g_weights = {}
		state_dict = torch.load(args.initial_checkpoint)['state_dict']
		for k,v in state_dict.items():
			if(k.find("gumbel")!=-1) and (k.find("blocks")!=-1):
				k = k.split('.')
				if k[1] in g_weights:
					g_weights[k[1]].append(v)
				else:
					g_weights[k[1]] = [v]
			elif (k.find("gumbel")!=-1):
				if "gumbel" in g_weights:
					g_weights["gumbel"].append(v)
				else:
					g_weights["gumbel"] = [v]
	...
	optimizer, gumbel_optimizer = create_optimizer_search(args, model)
	...
	gumbel_scheduler, num_epochs = create_scheduler(args, gumbel_optimizer)
	...
	if gumbel_scheduler is not None and start_epoch > 0:
		gumbel_scheduler.step(start_epoch)
	...
	try:  # train the model
		for epoch in range(start_epoch, num_epochs):
			...
			train_metrics = train_epoch(
				...
				lr_scheduler=lr_scheduler, gumbel_scheduler=gumbel_scheduler, saver=saver, output_dir=output_dir,
			...
			if gumbel_scheduler is not None:
				# step gumbel for next epoch
				gumbel_scheduler.step(epoch + 1, eval_metrics[eval_metric])
...
def train_epoch(
		epoch, model, loader, optimizer, gumbel_optimizer, loss_fn, args,
		lr_scheduler=None, gumbel_scheduler=None, saver=None, output_dir='', amp_autocast=suppress,
	...
	for batch_idx, (input, target) in enumerate(loader):
		if batch_idx < len(loader) * (1 - args.search_data_percent):
			train_network = True
		...
		with amp_autocast():
			output, latency = model(input, epoch)
			# output = updateSequenceOutput(output, 3)
			loss = loss_fn(output, target, latency)
