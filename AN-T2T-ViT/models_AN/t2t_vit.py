...
class T2T_ViT(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, token_dim=64, compression_factor=2, block_depth=2, DAN_training=False):
        super().__init__()
        ...
        if not DAN_training:
            ...
            for i in range(block_depth):
                compression_factors[i] = 1
                compression_factors[-1-i] = 1
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, depth=i, compression_factor=compression_factor[i], block_depth=block_depth, total_depth=depth)
            for i in range(depth)])
