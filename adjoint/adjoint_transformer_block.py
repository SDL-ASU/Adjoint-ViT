...
class AdjointMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., alpha=2, depth=0, block_depth=3):
        super().__init__()
...
class AdjointAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., alpha=2, depth=0, block_depth=3):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.depth >= self.block_depth:
            ...
            x_small = (attn_small @ v_small).transpose(1,
                                                       2).reshape(B//2, N, C)
...
class AdjointBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, depth=0, alpha=2, block_depth=3):
        super().__init__()
        self.norm1_large = norm_layer(dim)
        self.norm1_small = norm_layer(dim)
        self.alpha = alpha
        self.attn = AdjointAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias,
                                     qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, alpha=alpha, depth=depth, block_depth=block_depth)
        self.drop_path = DropPath(
            drop_path) if drop_path > 0. else nn.Identity()
        self.norm2_large = norm_layer(dim)
        self.norm2_small = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = AdjointMlp(in_features=dim, hidden_features=mlp_hidden_dim,
                              act_layer=act_layer, drop=drop, alpha=alpha, depth=depth, block_depth=block_depth)
        self.depth = depth
        self.block_depth = block_depth
