...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., alpha = 2, stage = 0, depth = 0):
        super().__init__()
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., alpha = 2, stage = 0, depth = 0):
    ...
    def forward(self, x, mask_large=None, mask_small=None):
        """
        Args:
        ...
        if (0 <= self.stage < 3 and self.stage + self.depth >= 1) or (self.stage == 3 and self.depth < 1):
            ...
            qkv_small[:, :, self.num_heads//self.alpha:] = 0

            q_small, k_small, v_small = qkv_small[0], qkv_small[1], qkv_small[2]  # make torchscript happy (cannot use tensor as tuple)

            q_small = q_small * self.scale
            attn_small = (q_small @ k_small.transpose(-2, -1))

            relative_position_bias_small = self.relative_position_bias_table[self.relative_position_index_small.view(-1)].view(
                self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
            relative_position_bias_small = relative_position_bias_small.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
            attn_small = attn_small + relative_position_bias_small.unsqueeze(0)

            if mask_small is not None:
                nW = mask_small.shape[0]
                attn_small = attn_small.view(B // (2 * nW), nW, self.num_heads, N, N) + mask_small.unsqueeze(1).unsqueeze(0)
                attn_small = attn_small.view(-1, self.num_heads, N, N)

            attn_small = self.softmax(attn_small)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 fused_window_process=True, alpha = 2, stage = 0, depth = 0):
        super().__init__()
        self.dim = dim
        ...
        self.stage = stage
        self.depth = depth
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, alpha=alpha, stage=stage, depth=depth)
    ...
    def forward(self, x):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask_large=self.attn_mask_large, mask_small=self.attn_mask_small)  # nW*B, window_size*window_size, C
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm, alpha=2, stage = 0, depth = 0):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 fused_window_process=False, alpha = [2], alpha_downsample=2, stage=0):
        ...
        if downsample is not None:
            self.downsample = downsample(input_resolution, dim=dim, norm_layer=norm_layer, alpha=alpha_downsample)
        else:
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, fused_window_process=False, alpha_downsample = [2, 2, 1, 1], alpha_l=[[1,2], [1,1], [8, 2, 8, 1, 4, 4, 1, 4, 1, 2, 2, 4, 2, 4, 8, 8, 1, 1], [4, 1]], **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               fused_window_process=fused_window_process,
                               alpha=alpha_l[i_layer],
                               alpha_downsample=alpha_downsample[i_layer],
                               stage=i_layer)
