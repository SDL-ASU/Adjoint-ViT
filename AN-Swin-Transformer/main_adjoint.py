...
def main(config):
	...
	for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
		...
		train_one_epoch(config, model, train_criterion, data_loader_train, optimizer, epoch, mixup_fn, lr_scheduler,
						loss_scaler)
		if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
			save_checkpoint(config, epoch, model_without_ddp, max_accuracy, max_adjoint_accuracy, optimizer, lr_scheduler, loss_scaler,
							logger)
...
def validate(config, data_loader, model, criterion):
	...
	batch_time = AverageMeter()
	standard_loss_meter = AverageMeter()
	acc1_meter = AverageMeter()
	acc5_meter = AverageMeter()

	kl_mean_meter = AverageMeter()
	loss_meter = AverageMeter()

	acc1_adjoint_meter = AverageMeter()
	acc5_adjoint_meter = AverageMeter()
	...
	for idx, (images, target) in enumerate(data_loader):
		...
		standard_loss, kl_mean, loss = criterion(output, target)

		l, _ = output.shape
		acc1, acc5 = accuracy(output[:l//2], target, topk=(1, 5))
		acc1_adjoint, acc5_adjoint = accuracy(output[l//2:], target, topk=(1, 5))

		acc1 = reduce_tensor(acc1)
		acc5 = reduce_tensor(acc5)
		standard_loss = reduce_tensor(loss)
		acc1_adjoint = reduce_tensor(acc1_adjoint)
		acc5_adjoint = reduce_tensor(acc5_adjoint)
		loss = reduce_tensor(loss)
		kl_mean = reduce_tensor(kl_mean)

		standard_loss_meter.update(standard_loss.item(), target.size(0))
		acc1_meter.update(acc1.item(), target.size(0))
		acc5_meter.update(acc5.item(), target.size(0))

		loss_meter.update(loss.item(), target.size(0))
		acc1_adjoint_meter.update(acc1_adjoint.item(), target.size(0))
		acc5_adjoint_meter.update(acc5_adjoint.item(), target.size(0))
		kl_mean_meter.update(kl_mean.item(), target.size(0))
		...
		if idx % config.PRINT_FREQ == 0:
			...
			logger.info(
				...
				f'Standard Loss {standard_loss_meter.val:.4f} ({standard_loss_meter.avg:.4f})\t'
